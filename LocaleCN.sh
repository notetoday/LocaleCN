...
if [ "${release}" == "centos" ]; then
	...
	cd /etc
	rm -rf locale.conf
	wget https://raw.githubusercontent.com/notetoday/LocaleCN/master/locale.conf > /dev/null 2>&1
	...
	cd /etc/
	wget https://raw.githubusercontent.com/notetoday/LocaleCN/master/locale.gen > /dev/null 2>&1
	locale-gen
	cd /etc/default/
	wget https://raw.githubusercontent.com/notetoday/LocaleCN/master/locale.conf > /dev/null 2>&1
	cp locale.conf locale
...
echo "TG Group: https://t.me/Notestoday"
